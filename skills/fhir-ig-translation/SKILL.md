...
Verified with **IG Publisher 2.2.11** and `fhir2.base.template` 0.1.0 (2026-07). This is publisher
behaviour and applies to any language pair.
...
| **Breadcrumbs & titles of `pages:`-tree pages** | **No** | The publisher localizes titles of pages it generates itself (e.g. the artifacts list) but renders `pages:`-tree titles — and the "Table of Contents" breadcrumb label — in the default language on every variant (`titlelang` carries the same string per language). Cosmetic; needs an upstream publisher change — do not work around it with sibling pages or menu tricks |
