...
Usage:
  ...
  gen-page-title-po.py IG_JSON SEED_FILE LANG OUT_PO
    ...
    IG_JSON    fsh-generated/resources/ImplementationGuide-<id>.json (run `sushi .` first)
    SEED_FILE  translation seed, one `English Title => Deutscher Titel` per line;
...
def main(argv):
    ...
    try:
        ...
        print("ERROR: cannot read the ImplementationGuide resource: %s" % exc, file=sys.stderr)
        print("       Expected fsh-generated/resources/ImplementationGuide-<id>.json --",
              file=sys.stderr)
        print("       run `sushi .` in the module before generating the catalogue.",
              file=sys.stderr)
        return 2
    ...
    if not pages:
        print("ERROR: no pages found in %s" % ig_path, file=sys.stderr)
        print("       An empty catalogue is never a pass. Check that sushi-config.yaml",
              file=sys.stderr)
        print("       declares a `pages:` tree and that `sushi .` regenerated the resource.",
              file=sys.stderr)
        return 2
    ...
    if seed_path == "-":
        ...
        try:
            ...
            print("ERROR: cannot read the seed file %r: %s" % (seed_path, exc), file=sys.stderr)
            print("       A seed path that does not resolve is a SETUP ERROR, not an empty",
                  file=sys.stderr)
            print("       seed: continuing would emit a catalogue in which every title is",
                  file=sys.stderr)
            print("       silently untranslated. Fix the path, or pass `-` to say",
                  file=sys.stderr)
            print("       deliberately that there is no seed. Nothing was written.",
                  file=sys.stderr)
            return 2
    ...
    if os.path.isfile(out_path):
        try:
            ...
            print("ERROR: %s exists but cannot be read: %s" % (out_path, exc), file=sys.stderr)
            print("       Refusing to overwrite a catalogue whose content is unknown.",
                  file=sys.stderr)
            return 2
    ...
    print("== gen-page-title-po (%s -> %s) ==" % (src_lang, lang))
    print("IG:   %s" % ig_path)
    print("seed: %s" % seed_label)
    if seed_stats is not None:
        print("      %d line(s) read, %d entry/entries parsed, %d matched a page title "
              "(%d applied), %d unused" % (seed_stats["lines"], seed_stats["entries"],
                                           len(matched_seed), len(applied_seed),
                                           len(unused_seed)))
        for no, line in seed_stats["malformed"]:
            print("      IGNORED seed line %d (no ' => ' separator): %s" % (no, line))
        for title in unused_seed:
            print("      UNUSED seed entry (no such page title): %s" % title)
    print("%s: %d unit(s), %d translated, %d left for review"
          % (out_path, len(units), len(units) - len(untranslated), len(untranslated)))
    if from_existing:
        print("      %d translation(s) carried over from the existing catalogue "
              "(they win over the seed)" % from_existing)
    if foreign:
        print("      %d unit(s) not owned by this generator preserved verbatim" % len(foreign))
    if header_entry is not None:
        print("      gettext header entry preserved")
    for title in obsolete:
        print("      DROPPED (no longer in the pages: tree): %s" % title)
    for at, block in dropped_blocks:
        ...
        print("      NOT PRESERVED (free-standing comment block, generator owns the header): "
              "%s" % block[0].strip())
    for title, names in collisions:
        print("      COLLISION: %d pages share the title %r (%s)"
              % (len(names), title, ", ".join(names)))
    if collisions:
        print()
        print("gettext keys by msgid, so pages sharing an English title share ONE unit and")
        print("cannot be translated differently. Give them distinct titles in sushi-config.yaml")
        print("if they need distinct translations.")
    if untranslated:
        for src in untranslated:
            print("  UNTRANSLATED: %s" % src)
        print()
        print("Translate the msgstr of the entries above by hand -- they are pages the seed")
        print("did not cover (typically pages that are not menu entries). Until then they")
        print("render in %s." % src_lang)
        return 1
    return 0
