...
2. Then work **① Decisions**, **② Reviews**, **③ QA triage** in that order. Every item names its owner — skip the items that are not yours.
3. **Gate 0**, **Identity**, **Verification** and **Protocol** are evidence. Open them only to check an item or to look a term up.
4. Item ids are typeable and greppable (`DEC-3`, `REV-2`, `QA-5`, `FIX-1`) — quote the id when you answer.
5. Nothing is published and every applied change is revertible; but doing nothing is not neutral, so read each item's **If nobody acts**.
...
`target_page` or `RETIRED` + reason). Check C3 reads it: a page in neither is reported MISSING. Keep
the two consistent — the ledger is the one that is checked.
