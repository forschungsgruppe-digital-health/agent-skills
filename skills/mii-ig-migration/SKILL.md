...
4. **Unreplaced placeholders.** The template does not build until every `{{...}}` placeholder is
   ...
   accounted (step-5 material). **Census by EXCLUSION (`grep -rIl '{{' . --exclude-dir=.git`),
   never an `--include` allow-list:** `*.yaml|*.md|*.json|*.xml` misses `ig.ini`, whose slug
   killed the publisher (PROs try-run). The `sushi-config.yaml` header lists the real placeholders.
